...
switch (_iClass) do {
	default {
		...
		} count getArray (mm_cfgFile >> "cfgLoot" >> _iClass);
		_qty = 0;
			...
			if (_index > 0) then {
				...
				if ((_item2 != "") && (isClass(configFile >> "CfgWeapons" >> _item2))) then{
					...
					if ((count _mags) > 0) then{
						...
						if (!(_item2 in ["MeleeFishingPole","MeleeCrowbar","MeleeBaseBallBatNails","MeleeBaseBallBatBarbed","MeleeBaseBallBat","Crossbow_DZ","MeleeSledge","MeleeMachete","MeleeHatchet_DZE"])) then{
	...
	case "single":
	{
		...
		{
			_itemTypes set[count _itemTypes, _x select 0]
		} count getArray(mm_cfgFile >> "cfgLoot" >> _iItem);
	...
	case "backpack":
	{
		...
		_itemTypes = [];
		_itemTypes = ((getArray (mm_cfgFile >> "cfgLoot" >> _iItem)) select 0);
		_index = dayz_CLBase find _iItem;
		_weights = dayz_CLChances select _index;
		_cntWeights = count _weights;
	    _index = floor(random _cntWeights);
	...
	case "weapon":
	{
		...
		if ((count _mags) > 0) then
		{
			...
			if (_mags select 0 == "30Rnd_556x45_G36") then { _mags set [0, "30Rnd_556x45_Stanag"] };
			if (_mags select 0 == "30Rnd_556x45_G36SD") then { _mags set [0, "30Rnd_556x45_StanagSD"] };
			if (!(_iItem in ["MeleeFishingPole","MeleeCrowbar","MeleeBaseBallBatNails","MeleeBaseBallBatBarbed","MeleeBaseBallBat","Crossbow_DZ","MeleeSledge","MeleeMachete","MeleeHatchet_DZE"])) then {
				_magQty = round(random 10);
				if (_magQty > 3) then {
					_item addMagazineCargoGlobal [(_mags select 0), (round(random 1) + 1)];
				};
